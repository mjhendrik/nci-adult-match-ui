...
export class CliaDartmouthComponent {
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < this.tablePCData.variant_reports.length; i++) {
      this.tablePCData.variant_reports[i].specimenReceivedDate = gmt.transform(this.tablePCData.variant_reports[i].specimenReceivedDate);
    }

    for (let i = 0; i < this.tablePCData.variant_reports.length; i++) {
      this.tablePCData.variant_reports[i].ngsDateReceived = gmt.transform(this.tablePCData.variant_reports[i].ngsDateReceived);
    }

    for (let i = 0; i < this.tablePCData.assignment_reports.length; i++) {
      this.tablePCData.assignment_reports[i].dateAssigned = gmt.transform(this.tablePCData.assignment_reports[i].dateAssigned);
    }
